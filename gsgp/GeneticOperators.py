...
def crossover(rng, parents):
    ...
    ind.create(weights, rng, semantics=semantics, Tr_X=ind1.training_X, Tr_Y=ind1.training_Y, Te_X=ind1.test_X,
               Te_Y=ind1.test_Y)
...
def mutation(rng, parent, normalizedForest, mutation_step):
    ...
    for i in range(len(parent.semantics)):
        semantics.append(parent.semantics[i] + mutation_step * (
                normalizedForest[tr1].semantics[i] - normalizedForest[tr2].semantics[i]))

    ind = Individual(parent.operators, parent.terminals, parent.max_depth)
    ind.create(weights, rng, semantics=semantics, Tr_X=parent.training_X, Tr_Y=parent.training_Y, Te_X=parent.test_X,
               Te_Y=parent.test_Y)
