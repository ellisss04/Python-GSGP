...
class GSGP:
    ...
    def getBestIndividualRepresentation(self):
        ...
        for i in range(len(self.bestIndividual.weights)):
            if i < self.population_size:
                ...
                if self.bestIndividual.weights[i] != 0:
                    s += str(self.bestIndividual.weights[i]) + "  *  " + str(
                        self.normalizedForest[i - self.population_size]) + " + "
    ...
    def fit(self, Tr_x, Tr_y, Te_x=None, Te_y=None):
        ...
        for i in range(self.population_size):
            ...
            tmp.create(weight, self.rng, semantics=ind.getSemantics(), Tr_X=self.Tr_x, Tr_Y=self.Tr_y, Te_X=self.Te_x,
                       Te_Y=self.Te_y)
    ...
    def nextGeneration(self):
        ...
        while len(newPopulation) < self.population_size:
            offspring = getOffspring(self.rng, self.population, self.normalizedForest, self.tournament_size,
                                     self.mutation_step)
        ...
        if self.verbose and self.currentGeneration % 5 == 0:
            if not self.Te_x is None:
                print("   > Gen #%2d:  Fitness: %.6f // Tr-Score: %.6f // Te-Score: %.6f  // Time: %.4f" % (
                    self.currentGeneration, self.bestIndividual.getFitness(),
                    self.bestIndividual.getAccuracy(self.Tr_x, self.Tr_y, pred="Tr"),
                    self.bestIndividual.getAccuracy(self.Te_x, self.Te_y, pred="Te"), end - begin))
            else:
